          ...
          <div className="flex gap-4 items-center flex-1 min-w-0">
            ...
            <motion.div onClick={() => setActive(card)} layoutId={`image-${card.name}-${id}`} className="flex-shrink-0">
              <img
                ...
                src={`${card.avatar}`}
                alt={card.name}
                className="h-12 w-12 sm:h-14 sm:w-14 rounded-lg object-cover object-top" />
            </motion.div>
          ...
          <div className="flex flex-row items-center gap-2 sm:gap-4">
              ...
              <>
                <div className="flex items-center gap-1">
                  <button 
                    ...
                    className={`w-6 h-6 sm:w-8 sm:h-8 flex items-center cursor-pointer justify-center rounded-full ${pointsCounter[card._id] > 0 ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-400'}`}
                  ...
                  <div className="w-8 sm:w-10 text-center font-medium text-xs sm:text-sm">
                    {pointsCounter[card._id] || 0}
                  </div>
                  
                  <button 
                    onClick={(e) => incrementPoints(card._id, e)}
                    className="w-6 h-6 sm:w-8 sm:h-8 flex items-center justify-center rounded-full bg-green-100 text-green-600"
                ...
                <span
                  ...
                  className={`px-1 py-1 flex-shrink-0 ${processingCards[card._id] ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                >
                  <div className="inline-flex items-center justify-center shrink-0 w-6 h-6 sm:w-8 sm:h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200">
                    {processingCards[card._id] ? (
                      <svg className="w-3 h-3 sm:w-4 sm:h-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                      </svg>
                    ) : (
                      <svg className="w-3 h-3 sm:w-4 sm:h-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
            ...
            {confirmation && (
              <div className="flex items-center">
                <motion.button
                  ...
                  disabled={processingCards[card._id]}
                  className={`px-2 py-1 w-20 sm:w-28 text-xs sm:text-sm rounded-lg font-bold bg-[--primary] text-white ${
            ...
            {(confirmation || memberList) && 
              <span
                ...
                className={`px-1 py-1 flex-shrink-0 ${processingCards[card._id] ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}>
                <div className="inline-flex items-center justify-center shrink-0 w-6 h-6 sm:w-8 sm:h-8 text-red-500 bg-red-100 rounded-lg dark:bg-red-800 dark:text-red-200">
                  {processingCards[card._id] ? (
                    <svg className="w-3 h-3 sm:w-4 sm:h-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
